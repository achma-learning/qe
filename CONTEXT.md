...
- `high-yield.html` — root-level document browser (`QE.bootHighYield()`): lists every module (no quiz) and links/previews its high-yield analysis doc (PDF/Word/txt) from `qe-analysis/<slug>/`. Reads `window.QE_ANALYSIS` (baked by `tools/build-analysis.js`).
- `qe-analysis/<slug>/` — drop a PDF/Word/txt high-yield doc here (built by the user's external `qe-analysis` Claude skill). `qe-analysis/_analysis.js` is the auto-generated manifest (`window.QE_ANALYSIS`). `qe-analysis/README.md` maps slug→module.
...
- `.github/workflows/deploy.yml` — CI: on PRs validates `.txt` (`--strict`); on push to `main` bakes `data/*`, deploys the site to GitHub Pages, and best-effort commits the baked data back for the offline copy. **Stages files by an explicit allow-list** (`cp index.html report.html high-yield.html manifest.webmanifest sw.js icon.svg .nojekyll …` + `cp -r assets data modules qe-analysis`) — **any new root HTML/asset must be added there or it won't deploy.**
...
## 7. Current State
- **Last shipped:** **Auto-advance timer fix** — the per-question countdown now resets on every question change (exam *and* training) via a new `syncTimer()` guard (`idx`/`phase`/`!timerH`), instead of restarting on every re-render. Exam mode previously never restarted it (only `renderTraining` called `startTimer`); `renderExamRun` now sets `phase='question'` + calls `syncTimer()`. Side effect: picking an option no longer refills the countdown (consistent with `shownAt`). Verified with DOM-stub integration tests for both modes.
- **Earlier:** New **`high-yield.html`** page + `qe-analysis/<slug>/` folders + `tools/build-analysis.js` (manifest baker, wired into CI). Lists every module and previews/links its high-yield analysis doc (PDF/Word/txt) that the user uploads — produced by their external `qe-analysis` Claude skill. Topbar now has **Report** + **High-Yield** nav links (the active page's link is hidden). Deploy allow-list + SW shell (`CACHE` → `qe-v4`) updated.
- **Earlier:** Reset sync — the report is computed live from `answers.<slug>`/`exam.<slug>.*` (both wiped by `resetModuleProgress`), so resetting a module's progress empties its report (proven by test). Added `pageshow`(bfcache)/`storage`(cross-tab) listeners on `report.html` + the dashboard so the **displayed** report/cards refresh after a reset done elsewhere or restored via the back button. On-page-only **"Personalized Revision Plan"** (kept out of the copied prompt & `.txt`).
- **Earlier:** Smarter **AI report** (`report.html`): the generated prompt/.txt now leads with a computed **statistical analysis** (totals, wrong-vs-partial, per-module, per-topic frequency ranking), and instructs the AI to **classify each error** (🧠 connaissance / 🔗 raisonnement / 🪤 piège QCM) and surface **notions à forte rentabilité pédagogique**. On-page stats panel mirrors it. Also: offline-safe clipboard (`copyText`/`execCopy`), and the duplicate top-bar Report link is hidden on report.html.
