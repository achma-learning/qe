...
## 7. Current State
- **Last shipped:** New **`high-yield.html`** page + `qe-analysis/<slug>/` folders + `tools/build-analysis.js` (manifest baker, wired into CI). Lists every module and previews/links its high-yield analysis doc (PDF/Word/txt) that the user uploads — produced by their external `qe-analysis` Claude skill. Topbar now has **Report** + **High-Yield** nav links (the active page's link is hidden). Deploy allow-list + SW shell (`CACHE` → `qe-v4`) updated.
